...
const SpotifyuserProperty = require('./properties/spotifyuser-property');
const getMediaPath = require('./helpers').getMediaPath;

class RadioDevice extends Device {
  constructor(adapter, id, ip, name, sysmodelist, maxvolume, minfreq, stepfreq, maxfreq) {
    super(adapter, id);
    this.ip = ip;
    ...
    this.actionsfn = [];
    this.mediaDir = path.join(getMediaPath(this.adapter.userProfile.mediaDir), this.id);
    ...
    this.properties.set('netremote.play.frequency', new FrequencyProperty(this, minfreq, stepfreq, maxfreq));
    this.properties.set('info', new InfoProperty(this));
    this.properties.set('cover', new CoverProperty(this));
    this.properties.set('spotifyuser', new SpotifyuserProperty(this));
