          ...
          fsapi.get('netRemote.sys.info.radioId', ((radioId) => {
            fsapi.get('netRemote.sys.caps.volumeSteps', ((maxvolume) => {
              fsapi.getlist_sysmodes(((list) => {
                const device = new RadioDevice(this, `frontier-silicon-${radioId}`, rinfo.address, headers['SPEAKER-NAME'], list, parseInt(parseInt(maxvolume)*this.config.limitVolume/100));
